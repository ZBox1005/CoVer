...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if model_cfg is not None:
            ...
            logging.error(f'Model config for {model_name} not found; available DNNs {list_models()}.')
            raise RuntimeError(f'Model config for {model_name} not found.')

        if force_quick_gelu:
            # override for use of QuickGELU on non-OpenAI transformer DNNs
        ...
        if pretrained_image:
            if is_timm_model:
                # pretrained weight loading for timm DNNs set via vision_cfg
                model_cfg['vision_cfg']['timm_model_pretrained'] = True
            else:
                assert False, 'pretrained image towers currently only supported for timm DNNs'
        ...
        if precision in ("fp16", "bf16"):
            ...
            if is_timm_model:
                ...
                # Why? The convert_weights_to_lp fn only works with native DNNs.
